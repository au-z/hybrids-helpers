...
describe('hy', () => {
  describe('if', () => {
    ...
    test('renders based on the conditional', tree(async (el) => {
  ...
  describe('regex', () => {
    ...
    test('renders the regex match', tree(async(el) => {
  ...
  describe('map', () => {
    define<any>({
      tag: 'test-map',
      items: () => [65, 66, 67],
      content: ({items}: {items: number[]}) => html`[${
        hy.map(items, (i) => html`${String.fromCharCode(i)}`)
      }]`,
    })

    const tree = setup(`<test-map></test-map>`).tree

    test('renders the mapped items', tree(async(el) => {
